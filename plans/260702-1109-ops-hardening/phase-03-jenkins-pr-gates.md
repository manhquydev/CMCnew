...
- `disableConcurrentBuilds()` (`Jenkinsfile:9`) already serializes on the 2-vCPU box → an extra per-PR Postgres won't
  cause parallel resource spikes; it lengthens queue time, acceptable.
...
- **CI minutes / queue time on 2 vCPU (MED×LOW):** more builds run the DB stage; serialized by `disableConcurrentBuilds`. Acceptable; monitor.
- **Port collision (LOW):** fixed `55432` fine while builds are serialized; if concurrency ever enabled, randomize the port.
