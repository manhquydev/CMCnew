...
## Key Insights
- **Model + unique constraint already exist** (`@@unique([facilityId, years])`, `schema.prisma:991`). NOTE the unique spans ARCHIVED rows too — re-adding a previously-archived year cannot insert a second row; the upsert must CLEAR `archivedAt` and OVERWRITE `percent` on the existing row in place. So archive is NOT a per-row historical trail. The real pricing-audit trail lives in `receipt.tierPercent` (frozen per receipt, `schema.prisma:1036`) + the audit log — that is where a past receipt's discount is reconstructed. State this so "archive keeps the audit trail" is not misread as row-level history.
...
- Mutation `finance.discountTierUpsert` must handle the archived-collision: upsert on `@@unique([facilityId, years])` clears `archivedAt` and overwrites `percent` in place (no second row).
- Mutation `finance.discountTierArchive({ id })`: set `archivedAt` (soft; re-adding the same years re-activates the same row in place); `logEvent`.
...
## Risk Assessment
- **Silent repricing (Med×High)**: editing a tier changes future receipt prices → mitigate with audit on every write + the defaults banner + the manual repricing check; APPROVED receipts are unaffected (netAmount/tierPercent stored, not recomputed). Caveat: a DRAFT priced under old tiers approves later at the STALE draft price — `receiptApprove` never reprices (`finance.ts:308-349`). Note this so the operator is not surprised by an in-flight draft holding an old tier.
