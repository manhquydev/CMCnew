# Decision 0032 — Student LMS login = parent phone (84xxx) + family profile picker, fixed default password

- Status: proposed (author = planner; approve before P1 implementation)
- Date: 2026-07-04 (RE-AUTHORED — supersedes the per-child-suffix design of the same number)
...
The prior 0032 design made the student login identity = the parent phone with a per-child
...
handled, not claimed away. **Handling (P1):** wrap the ParentAccount `create` in a SAVEPOINT and
...
New pure helper `normalizeLoginPhone(raw): string | null` in `packages/auth` (NOT `crm.ts`
...
### D3. Schema home = REUSE `ParentAccount.passwordHash` — NO migration (RECOMMENDED)
...
**P1 SECURITY REQUIREMENT (2nd red-team, BLOCKING — supersedes the prior "accepted transient
risk"):** the phone-login path must NEVER produce a parent-portal-capable session. The weak,
publicly-known default credential (parent phone is printed on receipts + fixed `Cmc2026@`) must
not grant access to the parent portal. Concretely, `parentProcedure` today gates only on
`kind==='parent'` (`trpc.ts:86-89`) and thus authorizes `guardian.profileUpdate`
...
- **Ticket vs `kind:'family'` — ticket recommended.** The ALTERNATIVE is adding a restricted
  `kind:'family'` to the `LmsSession.kind` union (`lms.ts:9`) that `signLmsSession` /
  `resolveLmsSession` support and that `parentProcedure` EXPLICITLY REJECTS (only `kind==='parent'`
  passes). The ticket approach is RECOMMENDED: smaller blast radius — it introduces a separate
  short-lived token consumed by exactly one endpoint and touches no existing `parentProcedure`
  consumer, whereas the `kind:'family'` union change must be audited against every place that
  reads `ctx.lms.kind`. Final choice confirmed at P1 implementation; either way the invariant is
  the same — a phone-login principal is REJECTED by every `parentProcedure` mutation.
...
- **loginCode scheme reconciliation (2nd red-team M1).** The break-glass loginCode is created in
  ...
  break-glass login staff relay to parents (Phase-2 ERP + Phase-3 copy) is consistent and
  globally unique. (P1 owns the `student.ts` edit alongside the random→default change.)
- **OPEN (red-team) — cascade revocation:** default design bumps ONLY `ParentAccount.tokenVersion`
  on change/reset. Already-entered child STUDENT sessions (minted via `enterChildProfile`) carry
  the child's `StudentAccount.tokenVersion` and are NOT revoked (valid ≤12h cookie life). To also
  kick out in-flight child sessions we would cascade-bump every guardianed child's
  `StudentAccount.tokenVersion`. Given student security is de-scoped, the default is NO cascade
  (YAGNI). Confirm before P1 whether family reset must revoke live child sessions.
  **Good property (keep):** bumping `ParentAccount.tokenVersion` DOES evict any live family/parent
...
- New auth-layer surface (P1 owns): the child-selection ticket sign/verify path in `packages/auth`
  (or, if the `kind:'family'` alternative is chosen, the `LmsSession.kind` union + the
  `parentProcedure` rejection). Either way the invariant is a P1 test gate: a phone-login principal
  is FORBIDDEN at every `parentProcedure` mutation.
...
- Provisioning's ParentAccount find-or-create is now race-safe (SAVEPOINT+`P2002` refetch or
  `ON CONFLICT DO NOTHING`+refetch) so a concurrent first-sibling approve cannot roll back the
  money tx.

## Open questions for human sign-off before P1

1. **Cascade revocation (D6 OPEN):** on family password change/reset, revoke in-flight child
   student sessions (cascade-bump StudentAccount.tokenVersion) or not (default: not)?
2. Confirm schema home = reuse `ParentAccount.passwordHash` (D3) — recommended, no migration.

## Durable-record TODO (at P0 implementation, not now)

Copy to `docs/decisions/0032-student-login-phone-identity.md` and register:
`scripts/bin/harness-cli.exe decision add …`.
