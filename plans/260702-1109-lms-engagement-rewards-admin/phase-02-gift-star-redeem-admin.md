...
| `starAdjust` bypasses redeem's `pg_advisory_xact_lock(hashtext(studentId))` (`rewards.ts:71`) — a concurrent `starAdjust -N` can commit between redeem's balance read and its `starTransaction.create`, dipping balance below what `checkRedeem` validated | L×M | **Accepted, documented (not silently missed):** star adjustments are rare, director-gated, fully audited manual corrections — they do NOT warrant the advisory lock the high-frequency redeem path needs. No invalid state results (negative balances already accepted above). Redeem's "no over-consume" charter (`rewards.ts:63`) holds against concurrent *redeems*, which is its actual threat model; it is not a cross-`starAdjust` invariant. Do not add the lock to `starAdjust` (YAGNI). |
