...
- **4 decision records** (next free id 0024; 0023 is latest):
  - 0024 commission: sale creates draft receipt from opportunity; receiptApprove auto-advances linked opp → O5_ENROLLED.
  - 0025 attendance-payroll: late/early punch penalty auto-deducted in payslipCompute, director-overridable pre-finalize.
...
| P4 | Attendance→payroll: penalty deduction into payslipCompute + override + monthlyReport proc/UI + history UI + withdraw + notif + delegated approver | pending | [phase-04-attendance-payroll.md](phase-04-attendance-payroll.md) |
...
- **`packages/auth/src/permissions.ts` + its parity snapshot** edited by P3 (finance.receiptCreate), P4 (shiftRegistration.approve/reject), P5 (afterSale.*). **Do NOT parallelize these three** — land edits in order P3 → P4 → P5, sequential commits, regen snapshot once at P6.
- `packages/db/prisma/schema.prisma` + migration: P1 only.
- P2 owns `apps/api/src/routers/user.ts`, `apps/admin/src/staff-profile.tsx`, `apps/admin/src/App.tsx` (UserCreateModal); EXTENDS `payroll.profileUpsert` forms wired by seam-fixes P5 — do not duplicate that form scaffold.
- P3 owns `apps/admin/src/finance-panel.tsx`, `apps/admin/src/opportunity-detail.tsx`, `apps/api/src/routers/finance.ts`.
- P4 owns `apps/api/src/routers/{check-in-out,payroll,shift-registration}.ts` + attendance/shift panels.
...
1. Sale chốt O4 → "Tạo phiếu thu" (draft, opportunityId auto-linked) → director/kế toán duyệt → commission soldById + kind=new correct + opp auto → O5 (int test via UI path, no hand-called API).
2. Monthly payslip has a late/early deduction line matching aggregated punches; director can override pre-finalize; finalize locks.
...
5. Team lead with managerId set approves that staff's shift packet; cannot self-approve.
