...
test.describe('admin HR panel', () => {
  test('Nhân sự & Lương NavLink is visible in admin sidebar after login', async ({ page }) => {
    await login(page);

    await expect(page.locator('nav').getByText('Nhân sự & Lương')).toBeVisible();
  });

  test('clicking Nhân sự & Lương NavLink renders the staff roster table', async ({ page }) => {
    await login(page);

    await page.locator('nav').getByText('Nhân sự & Lương').click();
