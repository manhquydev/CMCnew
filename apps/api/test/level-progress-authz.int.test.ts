...
// Only a head_teacher (or super_admin) may DECIDE. Approve writes the new level onto Student.level;
// a non-head_teacher attempting to decide → FORBIDDEN; reject leaves Student.level unchanged.
//
// The router exposes ONE decision procedure: levelProgress.decide({ id, decision: 'approve'|'reject' }),
// gated by requireRole(Role.head_teacher) (super_admin bypasses). propose is requireRole(giao_vien,
// head_teacher, quan_ly). Approve side-effects (in the same tx) also auto-issue a Certificate and a
// student Notification — afterAll cleans those FK-children before the student.
describe('level-progress propose/decide authz (Phase 2 §2.10 invariant)', () => {
  const FACILITY = 1;
  let studentId: string;

  // A facility-1 head_teacher: allowed to decide, and RLS-scoped to read the facility-1 proposal.
  const headTeacher = () =>
    staffCaller({ isSuperAdmin: false, facilityIds: [FACILITY], roles: [Role.head_teacher], primaryRole: Role.head_teacher });
  ...
  it('non-head_teacher (giao_vien) decide → FORBIDDEN, and Student.level stays L1', async () => {
    const lp = await withRls(SUPER, (tx) =>
  ...
  it('head_teacher approves → LevelProgress APPROVED and Student.level becomes L2', async () => {
    const lp = await withRls(SUPER, (tx) =>
      tx.levelProgress.findFirstOrThrow({ where: { studentId, status: 'pending' }, select: { id: true } }),
    );
  ...
  it('reject path: a new proposal rejected by head_teacher leaves Student.level unchanged', async () => {
