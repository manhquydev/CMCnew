...
| `teacher.cmcvn.edu.vn`, `apps/api/src/routers/teacher-lite.ts`, Teacher Lite UI surface | Teacher Lite may direct-create parent/student/enrollment for MVP, bypassing receipt/finance/CRM while preserving LMS login rules | `docs/decisions/0039-teacher-lite-direct-lms-mvp.md` | Accepted |
| `apps/admin/src/app-surface.ts`, `apps/admin/src/{edu,biz}-director-cockpit-panel.tsx`, `apps/api/src/routers/teacher-lite.ts` + `teacher-lite-crud.ts`, `apps/api/src/routers/audit.ts` (class_session), `packages/auth/src/permissions.ts` (student.update/teacherLite/guardian.parentUpdate) | Teacher Lite simplification: strip ERP-heavy nav, `teacherLite.*` bypass keeps RLS+audit+anti-escalation, widen student.update to GĐĐT, class_session audit surfaced, grading score+feedback only | `docs/decisions/0040-teacher-lite-simplification-api-bypass.md` | Accepted |
