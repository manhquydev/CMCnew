...
> `devteacher.cmcvn.edu.vn` + `devlms.cmcvn.edu.vn`) sau prod nginx qua network `cmcnew-edge`. DB/Redis dev cô lập hoàn toàn,
...
- Domain trỏ về server: `erp.cmcvn.edu.vn`, `teacher.cmcvn.edu.vn`, `hoc.cmcvn.edu.vn`.
- Đã tạo Entra app (M365) + Redirect URI prod:
  `https://erp.cmcvn.edu.vn/api/auth/sso/callback` và
...
curl -sI https://teacher.cmcvn.edu.vn/api/health        # 200
curl -s -o /dev/null -w '%{http_code}' https://teacher.cmcvn.edu.vn/                 # 200
```
Mechanical smoke:
```powershell
powershell -NoProfile -ExecutionPolicy Bypass -File scripts/verify-teacher-cmcvn-live-smoke.ps1
```
Lệnh này verify health/root của `erp`, `teacher`, `hoc` và ERP/teacher SSO-start redirect URI mà không follow sang Microsoft login.
Nó cũng gọi Microsoft authorize pre-login cho ERP/teacher và fail nếu Microsoft trả `AADSTS50011`
hoặc `AADSTS900971`; check này chứng minh redirect URI được Entra chấp nhận trước đăng nhập, nhưng
không thay thế smoke đăng nhập thật qua browser/MFA.

- Đăng nhập `admin@cmcvn.edu.vn` qua SSO → vào được (super_admin).
  Có thể chạy verifier operator-assisted khi có người thao tác Microsoft/MFA:
  ```powershell
  powershell -NoProfile -ExecutionPolicy Bypass -File scripts/verify-teacher-cmcvn-interactive-sso.ps1
  ```
  Để verify cả 2 staff host trong một browser session:
  ```powershell
  $env:SSO_ORIGINS='https://teacher.cmcvn.edu.vn,https://erp.cmcvn.edu.vn'
  powershell -NoProfile -ExecutionPolicy Bypass -File scripts/verify-teacher-cmcvn-interactive-sso.ps1
  ```
  Lệnh này mở browser headed, người vận hành tự đăng nhập Microsoft/MFA, rồi script verify callback
  quay về từng staff host, staff shell đã authenticated, và cookie `cmc.session` là host-scoped.
