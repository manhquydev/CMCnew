...
| TEACH-SHELL | Module rail + horizontal sub-tab bar in the unified admin staff shell (back-office + teaching-origin modules under one role-filtered nav; Plan D module/IA restructure, 2026-07-04 — supersedes the earlier flat grouped-NavLink sidebar). _apps/teaching retired; consolidated into apps/admin._ | no | no | yes — `apps/e2e/tests/unified-staff-shell.spec.ts` | no | changed | `apps/admin/src/shell.tsx`, `apps/admin/src/nav-modules.ts` |
